...
var GRAPH = [
    {
        ...
        "prepFn": function() {
            ...
            for (m = 1; m < SHOWS.length; m += 1) {
                if (SHOWS[m].live == 1) {
                    console.log(m,'city',SHOWS[m].city,'state',SHOWS[m].state);
                    tally[SHOWS[m].city + ', ' + SHOWS[m].state] = (
                        Number.isNaN(tally[SHOWS[m].city + ', ' + SHOWS[m].state]) ?
                        1 :
                        tally[SHOWS[m].city + ', ' + SHOWS[m].state] + 1
                    );
                }
            }
    ...
    },
    {
        ...
        "prepFn": function() {
            var sortme = Array();
            for (iter = 1; iter < SHOWS.length; iter+=1) {
                if (SHOWS[iter] && SHOWS[iter].movie && SHOWS[iter].epdate) {
                    showD = new Date(parseInt(SHOWS[iter].epdate.substring(0,4)), parseInt(SHOWS[iter].epdate.substring(5,7)) - 1, parseInt(SHOWS[iter].epdate.substring(8,10)));
                    movieD = new Date(parseInt(MOVIES[SHOWS[iter].movie].release_date.substring(0,4)), parseInt(MOVIES[SHOWS[iter].movie].release_date.substring(5,7)) - 1, parseInt(MOVIES[SHOWS[iter].movie].release_date.substring(8,10)));
                    days = (showD - movieD) / 86400000;
            ...
            var valueArray = Array();
            var showArray = Array();
            for (iter = 0; iter < sortme.length && iter < 25; iter+=1) {
        ...
        "prepFn": function() {
            ...
            for (iter = 1; iter < SHOWS.length; iter+=1) {
                if (SHOWS[iter] && SHOWS[iter].movie && MOVIES[SHOWS[iter].movie].genre_ids) {
                    for (n = 0; n < MOVIES[SHOWS[iter].movie].genre_ids.length; n+=1) {
                        this_one = MOVIES[SHOWS[iter].movie].genre_ids[n];
                        tally[this_one] = (Number.isNaN(tally[this_one]) ? 1 : tally[this_one] + 1);
                    }
                }
            }
            var sortme = Array();
            for (n = 0; n < GENRES.length; n+=1) {
                this_one = GENRES[n];
    ...
    },
    {
        ...
        "prepFn": function() {
            ...
            for (iter = 1900; iter < years.length; iter += 10) {
                if (years[iter] > 0) {
    ...
    },
    {
        ...
        "prepFn": function() {
            ...
            for (iter = 1900; iter < years.length; ++iter) {
                    ...
                    tooltips: {
                        callbacks: {
                            label: function(tooltipItem, data) {
                                var thisYear = tooltipItem.xLabel;
                                var tlist = [];
                                for (iter = 1; iter < SHOWS.length; ++iter)  {
                                    try {
    ...
    },
    {
        ...
        "prepFn": function() {
            ...
            for (n = 1; n < SHOWS.length; ++n)
                if (SHOWS[n] != null)
                    for (j in SHOWS[n].guests)
            ...
            for (n = 1; n < SHOWS.length; ++n)
                if (SHOWS[n] != null && SHOWS[n].movie != null) {
                    ...
                    for (j in cass) {
                        if (HOST[cass[j]]) {
                            mcount[cass[j]] = Number.isNaN(mcount[cass[j]]) ? 1 : mcount[cass[j]] + 1;
        ...
        "prepFn": function() {
            ...
            for (iter = 1; iter < SHOWS.length; ++iter) {
                if (SHOWS[iter] === null || SHOWS[iter].movie === null)
                    continue;
                for (j = 0; j < MOVIES[SHOWS[iter].movie].cast.length; ++j) {
            ...
            for (iter = 1; iter < SHOWS.length; ++iter) {
                if (SHOWS[iter] && SHOWS[iter].guests) {
                    for (n = 0; n < SHOWS[iter].guests.length; ++n) {
                        ...
                        guestCount[value] = Number.isNaN(guestCount[value]) ? 1 : guestCount[value] + 1;
